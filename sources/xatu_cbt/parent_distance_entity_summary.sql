-- Truly stale parent rate by entity group (excludes benign missed-slot cases)
...
SELECT
    ...
    quantileIf(0.50)(toUInt32(f.parent_distance), is_truly_stale) AS p50_distance,
    quantileIf(0.95)(toUInt32(f.parent_distance), is_truly_stale) AS p95_distance,
    maxIf(toUInt32(f.parent_distance), is_truly_stale) AS max_distance
FROM flagged f
